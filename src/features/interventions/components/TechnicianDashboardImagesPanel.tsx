    ...
    return (
      ...
      >
        ...
        <p className="text-[14px] font-medium text-slate-400">
          Sélectionnez une mission pour voir les photos du client
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className={`${GLASS_PANEL_BODY_SCROLL_COMPACT} flex flex-col gap-4 px-4 py-4`}>
        {images.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-[20px] border border-dashed border-slate-200 bg-white/50 px-6 py-10 text-center">
            <ImageIcon className="mb-3 h-8 w-8 text-slate-300" aria-hidden />
            <p className="text-[14px] font-medium text-slate-500">Aucune photo fournie par le client</p>
          </div>
        ) : (
          <motion.div 
            initial="hidden"
