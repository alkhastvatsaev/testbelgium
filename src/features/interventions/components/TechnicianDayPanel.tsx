...
export default function TechnicianDayPanel() {
  ...
  const getStatusDisplay = () => {
    ...
    if (isLate) {
      return { label: "En retard", color: "text-rose-500", bg: "bg-rose-500/10 border-rose-500/20" };
    }
    return { label: "À l'heure", color: "text-emerald-500", bg: "bg-emerald-500/10 border-emerald-500/20" };
  };
      ...
      <div className={`${GLASS_PANEL_BODY_SCROLL_COMPACT} flex flex-col gap-4 shrink-0`}>
        <div className="flex shrink-0 items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[19px] font-bold tracking-tight text-slate-900">Le Radar</h2>
              <p className="text-[13px] font-medium text-slate-500">Ma Journée</p>
            </div>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-[20px] border border-black/[0.06] bg-white px-4 py-4 shadow-[0_14px_36px_-18px_rgba(15,23,42,0.14)]">
          {loading ? (
            ...
            <p className="text-[13px] font-medium text-slate-500">Connectez-vous pour voir vos missions.</p>
          ) : (
            <div className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-end justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[13px] font-bold text-slate-700">{completedToday.length} / {totalToday} terminées</span>
                  </div>
              ...
              {nextMission ? (
                ...
                <div className="mt-2 flex flex-col items-center justify-center rounded-[16px] border border-dashed border-slate-200 bg-slate-50/50 p-6 text-center">
                  ...
                  <p className="text-[14px] font-bold text-slate-700">Journée terminée</p>
                  <p className="mt-1 text-[12px] font-medium text-slate-500">Aucune autre mission prévue pour aujourd'hui.</p>
                </div>
              )}
            </div>
          )}
