...
import { Shield, Phone, KeyRound, Loader2, AlertTriangle, ArrowRight } from 'lucide-react';
        ...
        if (user.phoneNumber && firestore) {
          try {
            ...
            if (userDoc.exists()) {
              ...
              const au = userDoc.data() as Record<string, unknown>;
              try {
                await syncClientPortalProfile(user, {
                  whitelistCompanyId: companyIdFromAllowedUsersDoc(au),
                });
              } catch (e) {
                console.error("[LoginOverlay] syncClientPortalProfile (phone)", e);
              }
              setIsAuthenticated(true);
            } else {
    ...
    try {
      ...
      if (user && user.phoneNumber) {
        ...
        if (userDoc.exists()) {
          ...
          try {
            await syncClientPortalProfile(user, {
              whitelistCompanyId: companyIdFromAllowedUsersDoc(userData as Record<string, unknown>),
            });
          } catch (e) {
            console.error("[LoginOverlay] syncClientPortalProfile (OTP)", e);
          }
                ...
                devUiPreviewEnabled ? (
                  <div className="flex flex-col items-center justify-center gap-4 py-10 px-4">
                    <p className="text-center text-sm text-slate-500">
                      En développement la prévisualisation contourne normalement cet écran.
                    </p>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="rounded-xl bg-slate-900 px-6 py-3 text-sm font-bold uppercase tracking-widest text-white shadow-lg transition-all hover:scale-[1.02] hover:bg-slate-800 active:scale-95"
                    >
                      DEV ACCESS
                    </button>
                  </div>
                  ...
                  <div className="flex flex-col gap-4 px-5 pb-10 pt-6">
                    <div className="flex flex-col items-center gap-2 text-center">
                      ...
                      <h2 className="text-lg font-bold text-slate-900">Centre de dispatch</h2>
                      <p className="text-sm text-slate-500">
                        Connexion par SMS — numéro présent dans la liste autorisée Firestore.
                      </p>
                    </div>
                    ...
                    {!isConfigured || !auth ? (
                      <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
                        <div className="flex gap-2 font-semibold">
                          ...
                          Firebase non configuré sur ce déploiement
                        </div>
                        <p className="mt-2 text-amber-900/90">
                          Copiez les variables{" "}
                          <code className="rounded bg-amber-100/80 px-1 font-mono text-xs">NEXT_PUBLIC_FIREBASE_*</code>{" "}
                          depuis votre <code className="font-mono text-xs">.env.local</code> vers{" "}
                          <strong>Vercel → Project → Settings → Environment Variables</strong>, puis redéployez.
                        </p>
                      </div>
                    ) : (
                      <form onSubmit={handleSendSms} className="flex flex-col gap-4">
                        <label htmlFor="login-overlay-phone" className="sr-only">
                          Numéro de téléphone
                        </label>
                        <div className="relative">
                          <Phone className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" aria-hidden />
                          <input
                            id="login-overlay-phone"
                            type="tel"
                            autoComplete="tel"
                            value={phoneNumber}
                            onChange={(e) => setPhoneNumber(e.target.value)}
                            placeholder="+32 ou 0470…"
                            className="w-full rounded-2xl border border-slate-200 bg-white py-4 pl-12 pr-4 text-slate-900 shadow-inner outline-none ring-slate-900/10 placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/15"
                          />
                        </div>
                        <button
                          type="submit"
                          disabled={loadingState === "authenticating" || !phoneNumber.trim()}
                          className="flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-900 py-4 font-semibold text-white transition-colors hover:bg-slate-800 disabled:opacity-50"
                        >
